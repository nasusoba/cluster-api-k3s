...
const (
	...
	k3sAPIServerCertCommonName = "kube-apiserver"
)
...
type WorkloadCluster interface {
	...
	GetAPIServerCertificateExpiry(ctx context.Context, kthreesConfig *bootstrapv1.KThreesConfig, nodeName string) (*time.Time, error)
...
func (w *Workload) GetAPIServerCertificateExpiry(ctx context.Context, kthreesConfig *bootstrapv1.KThreesConfig, nodeName string) (*time.Time, error) {
	// Calculate the port the kube-apiserver is listening on.
	port, err := calculateAPIServerPort(kthreesConfig)
	if err != nil {
		return nil, errors.Wrapf(err, "unable to get certificate expiry for kube-apiserver on Node/%s: unable to get the api server port", nodeName)
	}
	...
	for _, cert := range conn.ConnectionState().PeerCertificates {
		if cert.Subject.CommonName == k3sAPIServerCertCommonName {
			kubeAPIServerCert = cert
		}
	}
	if kubeAPIServerCert == nil {
		return nil, errors.Wrapf(err, "unable to get certificate expiry for kube-apiserver on Node/%s: couldn't get peer certificate with cn=%q", nodeName, k3sAPIServerCertCommonName)
	}
...
// on a KThreesConfig.
func calculateAPIServerPort(config *bootstrapv1.KThreesConfig) (int64, error) {
	if config.Spec.ServerConfig.HTTPSListenPort != "" {
		return strconv.ParseInt(config.Spec.ServerConfig.HTTPSListenPort, 10, 32)
	}
